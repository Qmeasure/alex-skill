#!/usr/bin/env python3
"""渲染门：对已渲染的 PDF 断言空白页、首页溢出、首页元素齐全、页数区间。

这一项存在的理由：首页溢出把内容挤到第 2 页、留下一张近乎空白的页，
在开发过程中复发过两次，每次都只有肉眼看渲染图才发现。目视不是门，脚本才是。

用法：
    python check_render.py 报告.pdf                      # 完整报告，25–45 页
    python check_render.py 样张.pdf --sample --pages-min 3 --pages-max 3
    python check_render.py 报告.pdf --pre-ipo            # Pre-IPO 分支的首页元素
"""
...
COVER_LISTED = ["投资评级", "目标价", "当前价", "上行空间"]
COVER_PRE_IPO = ["申购建议", "合理市值区间", "拟募资额"]
...
def check(pdf, *, pages_min, pages_max, pre_ipo, sample):
    ...
    need = COVER_COMMON + (COVER_PRE_IPO if pre_ipo else COVER_LISTED)
    ...
    # 4. 首页未溢出：第 2 页剥掉页眉后，开头必须就是 Key data 标题。
    #    首页一旦溢出，第 2 页开头会变成封面的残留（表格孤儿行、口径小字等）。
    if n >= 2:
        head = norm(pages[1])
        if head.startswith(norm(RUNNING_HEADER)):
            head = head[len(norm(RUNNING_HEADER)):]
        ok2 = head[:20].startswith("Keydata")
        results.append((
            "首页未溢出", ok2,
            "第 2 页以 Key data 开头" if ok2
            else "第 2 页开头不是 Key data，疑似首页溢出：" + head[:40],
        ))
    else:
...
def main():
    ...
    ap.add_argument("--pre-ipo", action="store_true", help="首页按 Pre-IPO 分支的元素检查")
    ...
    results = check(args.pdf, pages_min=args.pages_min, pages_max=args.pages_max,
                    pre_ipo=args.pre_ipo, sample=args.sample)
