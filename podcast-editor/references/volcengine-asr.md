...
合成音轨启用说话人识别。服务返回的说话人编号不直接展示；按说话人第一次出现的顺序映射为 `speaker-1`、`speaker-2`，默认名称依次为“嘉宾一”“嘉宾二”。自动识别不是身份认证，短发言、相近音色和连续抢话都可能分错。审核页允许按发言轮次修正，修正保存在 `speakerOverrides`，原始返回和 `project.json` 不改。

多人分轨逐文件提交，每个文件固定映射到一个 `speakerId`。即使转录结果包含其他说话人编号，也不能据此把单个文件继续拆轨。

脚本使用 base64 直传本地音频，单个文件上限为 100 MiB。超过上限时停止并说明原因，不自动压缩，也不静默改用其他接口。
