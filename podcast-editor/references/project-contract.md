...
`retranscribe --project <目录>` 使用项目中已有的 `sources[].path` 重新建立逐字稿。候选项目必须先通过同一套 schema 校验，再替换原目录。项目路径、项目 ID、标题、原始音频路径、音频分析缓存和剪映草稿保持不变；新句段和词条使用本次重新识别专属的 ID，不能与旧项目复用编号。审核状态从新词条重新开始，旧试听和旧预选文件删除。
